...
- **runner** ([src/core/runner.ts](src/core/runner.ts)) — `expand()` turns a spec into cases (spec-level `vars`/`assert` are the base; each case merges its `vars` and appends its `assert`; no `cases` = one implicit case). An optional `scenario` (the BDD "Given") supplies a base `vars` layer and a `system` instruction; var precedence is `scenario.vars` → spec `vars` → case `vars`, and `system` is rendered per case with that case's vars. `mapLimit` bounds concurrency (default 5). A failed provider call becomes a `CaseResult` with `error`, never a thrown exception.
- **providers** ([src/providers/index.ts](src/providers/index.ts)) — `Resolver` maps a `ProviderSpec` → `{ provider, model }`. The `Provider` interface is just `complete({model, prompt, system?}) → {output, latencyMs, usage?}` (`system` is the scenario setup; each vendor maps it to its native system role — see [chatMessages](src/providers/http.ts)). Vendors register in the `VENDORS` map for the `vendor/model` string shorthand. `registerProvider()` is the public plugin point. **OpenAI-compatible vendors** (openai, glm, mistral, huggingface/hf, openrouter) all extend [OpenAICompatProvider](src/providers/openai-compat.ts) — a new named one is ~10 lines (vendor id + endpoint + key env). Prefer the generic `{ type: "openai-compat", baseUrl, model, apiKeyEnv }` object form (handled in `resolveObject`) for any host without a shorthand — no code change needed. Azure is separate (deployment-in-URL, `api-key` header, api-version); Google/Ollama/exec have their own shapes.
